...
export class Generator {
  ...
  async generateDockerfile(config) {
    ...
    lines.push('# 1. 此 Dockerfile 假设所有构建已在本地完成');
    lines.push('# 2. 不会在容器内执行任何编译/打包命令');
    lines.push('# 3. 如果缺少构建产物，请先在本地执行构建命令');
    lines.push('# ============================================================');
    lines.push('');
    ...
    // Copy frontend
    if (project.frontend) {
      lines.push('# Frontend static files (pre-built locally)');
    ...
    // Copy backend
    if (project.backend) {
      lines.push('# Backend application (pre-built locally, NOT compiled in container)');
